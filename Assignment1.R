...
library(ggplot2)
library(readr)
diamondsData <- read_csv("Desktop/Applied Analytics/App Anlytics Framework 1/Assignments/diamonds_data.csv")
View(diamondsData)

mean(diamondsData$carat)
library(dplyr)
filter(diamondsData$cut == "Premium")
?dplyr::filter()
filteredData <- dplyr::filter(diamondsData,grepl('Premium', cut))
filteredData
mean(filteredData$carat)
?group_by
diamondsData %>% group_by(cut) %>% var(diamondsData$carat)
